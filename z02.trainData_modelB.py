...
outdir = '/scratch/project_2003154/MachineLearning/FirstImages5TeV/'
for fn in glob(outdir+"results*.npz"):#[:3]:
	io = np.load(fn);
	data = io['images']
	obs = io['flowdata']
...
# Compile model
model_cnn.compile(optimizer=keras.optimizers.Adam(), loss=tf.keras.losses.MeanSquaredError());
